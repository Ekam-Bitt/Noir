...
export function CartDrawer() {
  ...
  return (
    ...
    >
      ...
      <aside
        className={`absolute right-0 top-0 flex h-full w-full max-w-md flex-col border-l border-white/10 bg-[#100d0c] p-5 shadow-2xl transition duration-300 md:p-6 ${isDrawerOpen ? "translate-x-0" : "translate-x-full"}`}
      >
        <div className="mb-6 flex items-center justify-between border-b border-white/8 pb-5">
          <div>
            ...
            <h2 className="text-2xl text-[#f5efe8]">Your selection</h2>
          </div>
          <button
            type="button"
            className="rounded-full border border-white/10 px-3 py-1 text-[11px] uppercase tracking-[0.2em] text-[#c7b9ab]"
            onClick={closeDrawer}
          >
            Close
          </button>
        ...
        <div className="flex-1 space-y-4 overflow-y-auto">
          {cart.lines.length ? (
            cart.lines.map((line) => (
              <div key={line.id} className="border-b border-white/8 pb-4">
            ...
            <div className="border border-dashed border-[#312b28] px-6 py-10 text-center">
